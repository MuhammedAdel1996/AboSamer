...
namespace Technical.Controllers
{
    ...
    public class CheckController : ControllerBase
    {
        ...
        {
            var result = _CheckRepositry.GetAll().Where(s => _CheckResultRepositry.GetAll().Where(x => x.orderid == s.id).Count() > 0 && ( role == 1 || s.Done == false || (s.count > 0 && s.create.AddHours(s.count) <= DateTime.Now))).Select(s => s.customerid).ToList().Distinct();
